...
>(function CodeBlock(
  ...
  const cmViewRef =
    useRef<CodeMirrorView | null>(null);
  ...
  const onUndo = useEditorEventCallback(
    (view) => {
      return undo(
        view.state,
        view.dispatch,
  ...
  const onRedo = useEditorEventCallback(
    (view) => {
      return redo(
        view.state,
        view.dispatch,
  ...
  const withMaybeEscape =
    useEditorEventCallback(
      (
        ...
        let main:
          | SelectionRange
          | Line = state.selection.main;
        if (!main.empty) {
          return false;
        }

        if (unit == "line") {
          main = state.doc.lineAt(
            main.head
          );
        }
